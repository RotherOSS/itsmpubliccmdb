...
sub PublicNavigationBar {
    ...
    # create menu items
    my %NavBarModule;
    my $FrontendModule   = $ConfigObject->Get('PublicFrontend::Module');
    my $NavigationConfig = $ConfigObject->Get('PublicFrontend::Navigation');
    ...
    if ( ref $ConfigObject->Get('PublicFrontend::NavBarModule') eq 'HASH' ) {
        my %Jobs = %{ $ConfigObject->Get('PublicFrontend::NavBarModule') };
    ...
    my $FrontendNotifyModuleConfig = $ConfigObject->Get('PublicFrontend::NotifyModule');
    ...
    $Param{URLLogo}   = $WebPath . 'skins/Public/default/img/otobo_logo_simple_w.svg';
    ...
    return $Self->Output(
        TemplateFile => 'PublicNavigationBar',
        Data         => \%Param,
    );
